...
# Menu con icone stilizzate e moderne
menu_options = {
        ...
        for client_id, c_name, c_group, c_address, c_phone in clients_data:
            if search_query in c_name.lower() or search_query in (c_group or "").lower():
                with st.container():
                    st.markdown(f"""
                        <div class='card'>
                            <h3 style='margin-bottom: 10px;'>🏢 {c_name}</h3>
                            <p style='color: #94a3b8; margin: 0;'><b>Commessa/Gruppo:</b> {c_group or 'N/D'} &nbsp;|&nbsp; <b>Indirizzo:</b> {c_address or 'N/D'} &nbsp;|&nbsp; <b>Tel:</b> {c_phone or 'N/D'}</p>
                        </div>
                    """, unsafe_allow_html=True)
                    
                    if user["role"] == "amministratore":
                        if st.button(f"🗑️ Elimina Cartella", key=f"del_{client_id}"):
                            conn = get_db_connection()
                            cursor = conn.cursor()
