...
def init_db():
    ...
    # 1. Imposta o aggiorna l'utente principale DEV (Enrico Bottone)
    dev_pin_raw = "enricobottone@"
    dev_pin_hash = hashlib.sha256(dev_pin_raw.encode()).hexdigest()
    ...
    if not cursor.fetchone():
        cursor.execute("INSERT INTO users (username, name, pin, raw_pin, role) VALUES (?, ?, ?, ?, ?)", 
                       ("DEV", "Enrico Bottone", dev_pin_hash, dev_pin_raw, "sviluppatore"))
    else:
        cursor.execute("UPDATE users SET name = 'Enrico Bottone', pin = ?, raw_pin = ?, role = 'sviluppatore' WHERE username = 'DEV'", (dev_pin_hash, dev_pin_raw))

    # 2. Imposta o aggiorna l'utente principale Salvatore (Salvatore Tammaro)
    salvatore_username = "Salvatore"
    salvatore_name = "Salvatore Tammaro"
    salvatore_pin_raw = "TSimpianti@"
    salvatore_pin_hash = hashlib.sha256(salvatore_pin_raw.encode()).hexdigest()
    
    # Pulisci eventuali vecchie scritture errate (es. 'SALVATORE')
    cursor.execute("DELETE FROM users WHERE username = 'SALVATORE'")
    
    cursor.execute("SELECT * FROM users WHERE username = ?", (salvatore_username,))
    if not cursor.fetchone():
        cursor.execute("INSERT INTO users (username, name, pin, raw_pin, role) VALUES (?, ?, ?, ?, ?)", 
                       (salvatore_username, salvatore_name, salvatore_pin_hash, salvatore_pin_raw, "amministratore"))
    else:
        cursor.execute("UPDATE users SET name = ?, pin = ?, raw_pin = ?, role = 'amministratore' WHERE username = ?", (salvatore_name, salvatore_pin_hash, salvatore_pin_raw, salvatore_username))
...
# --- FUNZIONE SUPPORTO PER APRIRE / VISUALIZZARE PDF CORRETTAMENTE ---
def display_pdf_link(pdf_path, key_suffix=""):
    if os.path.exists(pdf_path):
        ...
        custom_html = f'''
            ...
            <button onclick="openPdf_{key_suffix}()" style="
                width: 100%;
            ...
            ">🌐 &nbsp; Apri PDF</button>
        '''
        st.components.v1.html(custom_html, height=45)


# --- FUNZIONE GENERAZIONE PDF CON FOTO INCLUSA ---
def generate_pdf(report, client):
    ...
    c.drawString(50, height - 145, f"Durata Servizio: {report.get('duration', 'N/D')}")
    c.drawString(50, height - 160, f"Tipo Intervento: {report['type']}")
    c.drawString(50, height - 175, f"Tecnici Incaricati: {report['technician']}")
    c.drawString(50, height - 190, f"Compilato da: {report['author']}")
    ...
    c.setFont("Helvetica-Bold", 11)
    c.drawString(50, height - 225, "DESCRIZIONE DEI LAVORI ESEGUITI:")
    c.setFont("Helvetica", 10)
    
    text_object = c.beginText(50, height - 245)
    ...
    for expense in expenses_list:
        ...
        y_pos -= 15
        if expense.get('photo') and os.path.exists(expense['photo']):
            photo_to_draw = expense['photo']
    ...
    if photo_to_draw:
        try:
            c.setFont("Helvetica-Bold", 11)
            c.drawString(50, y_pos - 35, "FOTO ALLEGATA:")
            c.drawImage(photo_to_draw, 50, y_pos - 175, width=180, height=120, preserveAspectRatio=True, anchor='nw')
        except Exception as e:
            pass

    c.line(50, 90, 200, 90)
    c.drawString(50, 75, "Firma Tecnico")

    c.line(350, 90, 500, 90)
    c.drawString(350, 75, "Firma Cliente")
...
if not st.session_state.logged_user:
    ...
    with col2:
        with st.form("login_form"):
            ...
            # NOTA: Qui non forziamo .upper() per consentire l'inserimento esatto con la sola S maiuscola (es. "Salvatore")
            username_input = st.text_input("Nome Utente (es. Salvatore)").strip()
            pin_input = st.text_input("Password / PIN", type="password")
...
# Voci riservate esclusivamente allo Sviluppatore o all'Amministratore
if user["role"] in ["sviluppatore", "amministratore"]:
    menu_options["👥 &nbsp; Gestione Utenti"] = "Gestione Utenti"
    menu_options["⚙️ &nbsp; Gestione Backup & DB"] = "Gestione Backup"
                            ...
                            for rep_num, rep_date, rep_tech, rep_type, rep_desc, rep_exp, rep_auth in client_reports:
                                st.markdown(f"""
                                    <div style='background: rgba(15, 23, 42, 0.5); padding: 16px; border-radius: 12px; border: 1px solid rgba(255, 255, 255, 0.05); margin-bottom: 12px;'>
                                        ...
                                        <p style='margin: 4px 0; color: #cbd5e1; font-size: 0.9rem;'><b>Data:</b> {rep_date} &nbsp;|&nbsp; <b>Tecnici:</b> {rep_tech}</p>
                                ...
                                with col_dl:
                                    with open(pdf_path, "rb") as f:
                                        st.download_button(
                                            label=f"📥 Scarica PDF N. {rep_num}",
                                            data=f, file_name=pdf_path, mime="application/pdf",
                                            key=f"dl_rep_{rep_num}"
                                        )
                                with col_open:
                                    display_pdf_link(pdf_path, key_suffix=f"rep_{rep_num}")

                    if user["role"] in ["amministratore", "sviluppatore"]:
                        if st.button(f"🗑️ Elimina Cartella Cliente e Rapportini: {c_name}", key=f"del_{client_id}"):
                            conn = get_db_connection()
                            cursor = conn.cursor()
                            cursor.execute("DELETE FROM clients WHERE id = ?", (client_id,))
                            cursor.execute("DELETE FROM reports WHERE client = ?", (c_name,))
                            conn.commit()
                            cursor.close()
                            conn.close()
                            st.success(f"Cartella cliente '{c_name}' e relativi rapportini eliminati con successo.")
                            st.rerun()
...
# ==========================================
    ...
    # Preleviamo l'elenco dei tecnici
    cursor.execute("SELECT name FROM users WHERE role IN ('collaboratore', 'amministratore')")
    db_users = cursor.fetchall()
    cursor.close()
    conn.close()
    
    available_technicians = [u[0] for u in db_users]
    ...
    if not db_clients:
        ...
        with st.form("new_report_form"):
            ...
            raw_report_date = st.date_input("Data Esecuzione", datetime.today())
            report_date = raw_report_date.strftime("%d/%m/%Y")
            
            report_duration = st.text_input("Durata del Servizio (es. 2 ore, 09:00 - 11:30)")
            
            selected_technicians = st.multiselect(
                "Tecnici Incaricati (seleziona uno o più operatori)", 
                options=available_technicians,
                default=[user["name"]] if user["name"] in available_technicians else []
            )
            ...
            st.markdown("---")
            st.subheader("Spese Vive / Materiali e Foto")
            expense_item = st.text_input("Voce Spesa o Materiale")
            expense_amount = st.number_input("Importo (€)", min_value=0.0, step=0.5)
            
            uploaded_photo = st.file_uploader("Aggiungi Foto (Scatta o seleziona dalla galleria)", type=["jpg", "jpeg", "png"])

            submit_report = st.form_submit_button("Genera Rapportino e Salva")
            
            if submit_report:
                if not selected_technicians:
                    st.error("Seleziona almeno un tecnico incaricato!")
                else:
                    technicians_str = ", ".join(selected_technicians)
                    ...
                    if uploaded_photo is not None:
                        ...
                        photo_filename = f"uploads/foto_rep_{new_number}_{uploaded_photo.name}"
                        with open(photo_filename, "wb") as f_foto:
                            f_foto.write(uploaded_photo.getbuffer())

                    expenses_json = json.dumps([{
                        "item": expense_item, 
                        "amount": expense_amount, 
                        "photo": photo_filename
                    }] if expense_item or uploaded_photo else [])
                    
                    cursor.execute("INSERT INTO reports (number, client, date, duration, technician, type, description, expenses, author) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                                   (new_number, selected_client_name, report_date, report_duration, technicians_str, report_type, description, expenses_json, user["name"]))
                    conn.commit()
                    cursor.close()
                    conn.close()
                    ...
                    new_rep_dict = {
                        "number": new_number, "client": selected_client_name, "date": report_date,
                        "duration": report_duration, "technician": technicians_str, "type": report_type, 
                        "description": description, "expenses": expenses_json, "author": user["name"]
                    }
        ...
        if st.session_state.last_generated_pdf and os.path.exists(st.session_state.last_generated_pdf):
            ...
            col_dl_new, col_open_new = st.columns(2)
            with col_dl_new:
            ...
            with col_open_new:
                display_pdf_link(st.session_state.last_generated_pdf, key_suffix="new_rep")
...
# ==========================================
    ...
    cursor.execute("SELECT number, client, date, duration, technician FROM reports ORDER BY number DESC")
    ...
    for rep_num, rep_client, rep_date, rep_duration, rep_tech in all_reports:
        st.markdown(f"<div class='card'><b>N. {rep_num}</b> &nbsp;|&nbsp; Cliente: <b>{rep_client}</b> &nbsp;|&nbsp; Tecnici: {rep_tech} &nbsp;|&nbsp; Data: {rep_date}</div>", unsafe_allow_html=True)


# ==========================================
# 4. GESTIONE UTENTI (SVILUPPATORE / AMMINISTRATORE)
# ==========================================
elif selected_page == "Gestione Utenti":
    if user["role"] not in ["sviluppatore", "amministratore"]:
        st.error("Accesso negato. Sezione riservata.")
    else:
        st.title("👥 Gestione Utenti")
        
        with st.form("new_user_form"):
            st.subheader("➕ Aggiungi Nuovo Utente")
            new_username = st.text_input("Nome Utente")
            new_name = st.text_input("Nome e Cognome Completo")
            new_pin = st.text_input("Password / PIN", type="password")
            new_role = st.selectbox("Ruolo", ["collaboratore", "amministratore", "sviluppatore"])
        ...
        for uname, u_name, u_raw_pin, u_role in db_users:
            ...
            with col_info:
                st.markdown(f"<b>{u_name}</b> (Username: <code>{uname}</code>) - Password: <code>{u_raw_pin}</code> - Ruolo: <em>{u_role}</em>", unsafe_allow_html=True)
            with col_del:
                if uname not in ["DEV", "Salvatore"] and st.button("🗑️ Elimina", key=f"del_u_{uname}"):
                    conn = get_db_connection()
                    cursor = conn.cursor()
...
# ==========================================
# 5. GESTIONE BACKUP & DB (SVILUPPATORE / AMMINISTRATORE)
# ==========================================
elif selected_page == "Gestione Backup":
    if user["role"] not in ["sviluppatore", "amministratore"]:
        st.error("Accesso negato. Sezione riservata.")
    else:
        st.title("⚙️ Gestione Avanzata Database & Backup")
        st.write("Area tecnica per il controllo dei backup.")
        
        st.markdown("### 1. Scarica Backup Corrente")
        ...
        st.markdown("### 2. Ripristina Backup")
        uploaded_db = st.file_uploader("Seleziona il file `gestionale.db` da caricare", type=["db", "sqlite"])
        if uploaded_db is not None:
            if st.button("🔄 Conferma e Sostituisci Database", use_container_width=True):
                with open(DB_FILE, "wb") as f:
                    f.write(uploaded_db.getbuffer())
                st.success("Database ripristinato con successo! Ricarico la pagina...")
                st.rerun()
