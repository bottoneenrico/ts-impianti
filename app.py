...
def init_db():
    ...
    # Imposta o aggiorna l'utente principale DEV (Enrico Bottone) come Sviluppatore con la sua password originale
    dev_pin_raw = "enricobottone@"
    dev_pin_hash = hashlib.sha256(dev_pin_raw.encode()).hexdigest()
    
    # Puliamo eventuale vecchio utente ADMIN se presente
    cursor.execute("DELETE FROM users WHERE username = 'ADMIN'")
    
    cursor.execute("SELECT * FROM users WHERE username = 'DEV'")
    dev_record = cursor.fetchone()
    if not dev_record:
...
# Voci riservate esclusivamente allo Sviluppatore
        ...
        for client_id, c_name, c_group, c_address, c_phone in clients_data:
            if search_query in c_name.lower() or search_query in (c_group or "").lower():
                with st.container():
                    ...
                    # SIA L'AMMINISTRATORE CHE LO SVILUPPATORE POSSONO ELIMINARE LE CARTELLE E I RELATIVI DATI
...
# ==========================================
    ...
    # Preleviamo l'elenco dei nomi degli utenti registrati (escludendo DEV se si vuole, oppure prendendo tutti tranne lo sviluppatore)
    cursor.execute("SELECT name FROM users WHERE username != 'DEV'")
    ...
    if not db_clients:
        ...
        with st.form("new_report_form"):
            ...
            # Selezione multipla dei tecnici presenti nel database
...
# ==========================================
# 4. GESTIONE UTENTI (SOLO SVILUPPATORE)
# ==========================================
elif selected_page == "Gestione Utenti":
    if user["role"] != "sviluppatore":
        st.error("Accesso negato. Sezione riservata allo sviluppatore.")
    else:
        st.title("👥 Gestione Utenti (Area Sviluppatore)")
        ...
        for uname, u_name, u_raw_pin, u_role in db_users:
            ...
            with col_info:
                st.markdown(f"<b>{u_name}</b> ({uname}) - Password: <code>{u_raw_pin}</code> - Ruolo: <em>{u_role}</em>", unsafe_allow_html=True)
            with col_del:
                if uname != "DEV" and st.button("🗑️ Elimina", key=f"del_u_{uname}"):
                    conn = get_db_connection()
                    cursor = conn.cursor()
...
# ==========================================
# 5. GESTIONE BACKUP & DB (SOLO SVILUPPATORE)
# ==========================================
