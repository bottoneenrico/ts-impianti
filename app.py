...
from reportlab.pdfgen::canvas import Canvas if False else __import__('reportlab.pdfgen.canvas', fromlist=['canvas']).canvas
...
def generate_pdf(report, client):
    ...
    c = __import__('reportlab.pdfgen.canvas', fromlist=['canvas']).canvas.Canvas(filename, pagesize=A4)
    width, height = A4
...
# ==========================================
# 5. GESTIONE BACKUP & RIPRISTINO (Solo Admin)
# ==========================================
    ...
    if user["role"] != "amministratore":
        ...
        st.write("Usa questa sezione **prima di fare un reboot o un aggiornamento** dell'app. Scarica il database e caricalo subito dopo per ripristinare tutti i dati in un click.")
        ...
        with col_bk1:
            ...
            st.write("Fai clic sul pulsante qui sotto prima di riavviare il server per salvare il file di database corrente.")
            
            if os.path.exists(DB_FILE):
                with open(DB_FILE, "rb") as f:
                    st.download_button(
                        label="📥 Scarica File Database (`gestionale.db`)",
                        data=f,
                        file_name="gestionale.db",
                        mime="application/octet-stream",
                        use_container_width=True
                    )
            else:
                st.warning("Nessun file di database trovato al momento.")
            st.markdown("</div>", unsafe_allow_html=True)
            
        with col_bk2:
            st.markdown("<div class='card'>", unsafe_allow_html=True)
            st.subheader("2. Ripristina Backup")
            st.write("Subito dopo aver fatto il reboot, carica qui il file di database salvato in precedenza.")
            
            uploaded_db = st.file_uploader("Carica file `gestionale.db`", type=["db"])
            if uploaded_db is not None:
                if st.button("🔄 Conferma e Ripristina Dati", use_container_width=True):
                    with open(DB_FILE, "wb") as f:
                        f.write(uploaded_db.getbuffer())
                    st.success("Database ripristinato con successo! Tutti i dati sono tornati al loro posto.")
                    st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
