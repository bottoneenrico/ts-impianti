...
def init_db():
    ...
    # Imposta o aggiorna l'utente admin principale con password 'enricobottone@'
    admin_pin_raw = "enricobottone@"
    admin_pin_hash = hashlib.sha256(admin_pin_raw.encode()).hexdigest()
    
    cursor.execute("SELECT * FROM users WHERE username = 'ADMIN'")
    admin_record = cursor.fetchone()
    if not admin_record:
        cursor.execute("INSERT INTO users (username, name, pin, raw_pin, role) VALUES (?, ?, ?, ?, ?)", 
                       ("ADMIN", "Enrico Bottone", admin_pin_hash, admin_pin_raw, "amministratore"))
    else:
        cursor.execute("UPDATE users SET pin = ?, raw_pin = ? WHERE username = 'ADMIN'", (admin_pin_hash, admin_pin_raw))
...
def display_pdf_link(pdf_path, key_suffix=""):
    if os.path.exists(pdf_path):
        ...
        # Metodo sicuro con JavaScript integrato per bypassare i blocchi dei browser moderni
        custom_html = f'''
            <script>
                ...
                with st.container():
                    with st.expander(f"🏢 {c_name} — Commessa: {c_group or 'N/D'}"):
                        ...
                        if not client_reports:
                            ...
                            for rep_num, rep_date, rep_tech, rep_type, rep_desc, rep_exp, rep_auth in client_reports:
                                ...
                                # Pulsanti affiancati: Scarica PDF e Apri PDF
                    ...
                    if user["role"] == "amministratore":
                        if st.button(f"🗑️ Elimina Cartella Cliente e Rapportini: {c_name}", key=f"del_{client_id}"):
                            ...
                            cursor.close()
                            conn.close()
                            st.success(f"Cartella cliente '{c_name}' e relativi rapportini eliminati con successo. Riepilogo aggiornato.")
                            st.rerun()
...
# ==========================================
    ...
    if user["role"] != "amministratore":
        ...
        uploaded_db = st.file_uploader("Seleziona il file `gestionale.db`", type=["db", "sqlite"])
