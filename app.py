                ...
                with st.container():
                    ...
                    if user["role"] == "amministratore":
                        if st.button(f"🗑️ Elimina Cartella Cliente e Rapportini: {c_name}", key=f"del_{client_id}"):
                            conn = get_db_connection()
                            cursor = conn.cursor()
                            # Elimina prima il cliente
                            cursor.execute("DELETE FROM clients WHERE id = ?", (client_id,))
                            # Elimina anche tutti i rapportini associati a questo cliente per aggiornare statistiche e gestione mensile
...
# ==========================================
    ...
    if not db_clients:
        ...
        with st.form("new_report_form"):
            ...
            # Formato data giorno-mese-anno (GG/MM/AAAA)
            ...
            # Possibilità di aggiungere foto (scattandole o selezionandole dalla galleria)
...
# ==========================================
    ...
    if user["role"] != "amministratore":
        ...
        # Blocco 1: Scarica Backup
        st.markdown("### 1. Scarica Backup Corrente")
        st.write("Fai clic sul pulsante qui sotto per scaricare il file di database sul tuo dispositivo.")
        ...
        # Blocco 2: Ripristina Backup
        st.markdown("### 2. Ripristina Backup")
        st.write("Carica il file precedentemente salvato per ripristinare tutti i dati all'istante.")
