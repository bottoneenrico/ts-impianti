...
def generate_pdf(report, client):
    ...
    c.drawString(50, height - 175, f"Tecnico Incaricato: {report['technician']}")
        ...
        with st.form("new_report_form"):
            ...
            if submit_report:
                ...
                cursor.execute("INSERT INTO reports (number, client, date, duration, technician, type, description, expenses, author) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                               (new_number, selected_client_name, report_date, report_duration, technician_name, report_type, description, expenses_json, user["name"]))
                conn.commit()
                cursor.close()
                conn.close()
                ...
                new_rep_dict = {
                    ...
                    "duration": report_duration, "technician": technician_name, "type": report_type, 
...
# ==========================================
    ...
    cursor.execute("SELECT number, client, date, duration, author FROM reports ORDER BY number DESC")
    ...
    for rep_num, rep_client, rep_date, rep_duration, rep_author in all_reports:
        st.markdown(f"<div class='card'><b>N. {rep_num}</b> &nbsp;|&nbsp; Cliente: <b>{rep_client}</b> &nbsp;|&nbsp; Data: {rep_date} &nbsp;|&nbsp; Durata: <em>{rep_duration or 'N/D'}</em></div>", unsafe_allow_html=True)
